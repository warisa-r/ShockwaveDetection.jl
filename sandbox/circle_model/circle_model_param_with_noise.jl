...
function fit_circle_with_improved_guess(xy)
    ...
    # Estimate the centroid of the points as the initial guess for (x0, y0)
    x0_guess = mean(x)
    y0_guess = mean(y)

    # Estimate the radius as the average distance from the centroid
    r_guess = mean(sqrt.((x .- x0_guess).^2 .+ (y .- y0_guess).^2))

    p0 = [x0_guess, y0_guess, r_guess]  # Improved initial guess
...
# Function to fit a circle with a random initial guess
function fit_circle_with_random_guess(xy)
    # Random initial guess for circle fitting
    p0 = rand(3)  # Random initial guesses for x0, y0, r
...
output_path = raw"C:\Users\eivan\Dropbox\My PC (DESKTOP-AB2UFDR)\Desktop\SEP_Shockwave\1"
...
for (i, params) in enumerate(param_combinations)
    ...
    # Save the plot to a file in the specified path
    savefig(p, "$(output_path)\\circle_benchmark_results_params_$(i)_noise.png")
end
