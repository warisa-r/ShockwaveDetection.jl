...
function cluster_shock_points_at_t(points, dbscan_algo::DBSCANAlgo)
    ...
    for dbscan_cluster in dbscan_result.clusters
        ...
        shock_cluster = [points[:,i] for i in dbscan_cluster.core_indices]
...
function cluster_shock_points(dbscan_algo::DBSCANAlgo, shock_positions_over_time, flow_data)
    ...
    # Initialize shock_clusters_over_time with a predefined size of nsteps, filled with empty vectors
    ...
    for t in 1:nsteps
        if isempty(shock_positions_over_time[t])
            ...
            push!(shock_clusters_over_time, [])
        else
            points = cartesian_index_to_xy(shock_positions_over_time[t], x, y)
            shock_clusters_t = cluster_shock_points_at_t(points, dbscan_algo)
            push!(shock_clusters_over_time, shock_clusters_t)
        end
    end
